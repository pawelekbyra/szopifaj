...
3. **Strona startowa `serowymichal.pl`** — statyczny HTML/CSS/JS (`/var/www/serowymichal/index.html` na serwerze, nginx serwuje bezpośrednio, bez osobnej usługi/buildu — świadomie prosto, to strona marketingowo-rejestracyjna, nie część aplikacji Medusa). Formularz "Załóż sklepik" woła po kolei `/auth/user/emailpass/register` → `/admin/sklepiki/self-signup` → pokazuje potwierdzenie i przekierowuje do `https://141-253-103-172.nip.io/app` (docelowy login). "Zaloguj się" to zwykły link do istniejącego panelu admina — landing page **nie** reimplementuje logowania (świadomie, mniej ruchomych części) — po zalogowaniu się tam użytkownik normalnie wpisuje e-mail/hasło, które właśnie założył.
   - Nginx vhost: `/etc/nginx/sites-available/serowymichal.conf`, na razie tylko port 80 (DNS jeszcze się propaguje w home.pl w momencie pisania tej notatki) — certbot/HTTPS do zrobienia w kolejnym kroku, gdy DNS zacznie wskazywać na `141.253.103.172`.
   - `ADMIN_CORS`/`AUTH_CORS` w `app/.env` rozszerzone o `https://serowymichal.pl,https://www.serowymichal.pl` (obok istniejącego `nip.io`).

4. **Przetestowane end-to-end na żywym serwerze** (nie tylko czytaniem kodu): rejestracja → self-signup → login → `GET /admin/sklepiki` pokazuje tylko własny sklepik → próba dostępu do cudzego `sales_channel` (istniejący "Sklepik Testowy A") **403**, izolacja RBAC działa jak oczekiwano dla nowo zakładanych kont. Dane testowe (`test-serowymichal-e2e@example.com`, `test-serowymichal-e2e2@example.com` + ich sklepik/rola/polityki) posprzątane ręcznie przez SQL po teście (rola `szopifaj` przez `docker exec szopifaj-postgres psql`) — `Sklepik Testowy A` z poprzednich sesji pozostawiony nietknięty.

**Do zrobienia (nie zrobione w tej sesji):** certbot dla `serowymichal.pl` po propagacji DNS (właściciel konfiguruje DNS w home.pl równolegle); ewentualny rate-limiting/captcha na publicznym self-signup, jeśli okaże się problemem przy realnym ruchu.
