...
- ⬜ **Warstwy 1+2 (middleware + hak na Knex) jeszcze nie zaimplementowane.** Bez nich RLS jest dziś **całkowicie przezroczyste** — żaden kontekst nigdy nie jest ustawiany przez aplikację, więc każde zapytanie widzi wszystko (tryb systemowy z automatu). To bezpieczny stan przejściowy (nic się nie psuje, nic dodatkowo nie ogranicza), ale **RLS jeszcze nie chroni niczego w praktyce** — to najbardziej ryzykowna, najmniej mechaniczna część całej pracy (przechwytywanie wewnętrznych metod klienta Knex, nieudokumentowane jako stabilne API — ta sama krucha natura co "patch" w typowych wdrożeniach, tyle że we własnym źródle zamiast `node_modules`). Następny krok.
