...
**Status:** 🟢 **Scoping RBAC faktycznie działa i zweryfikowany end-to-end (2026-07-18).** Poprzedni wpis statusu (2026-07-17) ogłaszał to samo, ale było to nieprawdą — w bazie nie istniała wtedy ani jedna zawężona (`resource_id`) polityka, więc "end-to-end" nigdy realnie nie przetestowało tego, co miało chronić. Sesja 2026-07-18 znalazła i naprawiła trzy niezależne błędy, które razem czyniły scoping martwym kodem (szczegóły w sekcji "RBAC / bezpieczeństwo" i `## Log` w `roadmap.md`). Po naprawie: `createSklepikWorkflow` nadaje właścicielowi zawężoną rolę, a próba dostępu do cudzego `sales_channel`/`product` faktycznie kończy się 403 (potwierdzone testem na żywym serwerze, nie tylko czytaniem kodu). Pozostaje: widget "przełącznik sklepiku" w panelu admina — **kod napisany 2026-07-18, ale przerwane przed pierwszym udanym buildem panelu, nigdy niewidziane w przeglądarce — patrz `## Log` w `roadmap.md`, część 4, dokładny stan i następny krok**. **Nowe 2026-07-18 (ten sam dzień, sesja z właścicielem):** publiczny self-signup + trójwarstwowa domena `serowymichal.pl` (bare = wizytówka, `sklepiki.serowymichal.pl` = formularz, `nazwa.serowymichal.pl` = każdy sklepik) zbudowane, wildcard DNS+SSL, przetestowane end-to-end na żywym serwerze — patrz sekcja "Public self-signup" niżej. Przy okazji naprawione dwa realne blockery: kolizja regionu przy drugim sklepiku dla Polski, i losowy sufiks doklejany do adresu sklepiku nawet dla unikalnych nazw (teraz tylko przy realnej kolizji).
...
7. **🔄 Zmienione 2026-07-18 (decyzja właściciela): rejestracja jest teraz PUBLICZNA i otwarta**, nie zamknięta/zaproszeniowa jak pierwotnie zakładano. Powód: strona startowa `serowymichal.pl` ma pozwalać każdemu założyć własny sklepik od razu, bez zaproszenia — patrz sekcja "Public self-signup" niżej. Zaproszenia (`/admin/invites`) zostają dostępne jako dodatkowa ścieżka (np. właściciel platformy zapraszający kogoś bezpośrednio z panelu), ale nie są już jedyną drogą zakładania konta.
...
### Public self-signup + strona startowa serowymichal.pl (2026-07-18)

**Zrobione i przetestowane end-to-end na żywym serwerze:**

1. **`POST /admin/sklepiki/self-signup`** (`packages/medusa/src/api/admin/sklepiki/self-signup/route.ts` + workflow `packages/medusa/src/workflows/sklepik/self-signup.ts`) — publiczny (bez zaproszenia) odpowiednik `acceptInviteWorkflow`: przyjmuje actorless token z `POST /auth/user/emailpass/register` (nagłówek `Authorization: Bearer <token>`), tworzy `User` (`createUsersWorkflow`), linkuje go do tożsamości auth (`setAuthAppMetadataStep`), po czym uruchamia istniejący `createSklepikWorkflow` żeby założyć sklepik i nadać rolę właściciela. Wymaga własnego wpisu w `packages/medusa/src/api/admin/sklepiki/middlewares.ts` z `authenticate("user", ["bearer"], { allowUnregistered: true })` (ten sam wzorzec co `/admin/invites/accept`) — samo `export const AUTHENTICATE = false` w pliku route.ts **nie wystarcza** bez tego wpisu, inaczej `req.auth_context` przychodzi `undefined` (znalezione i naprawione tą sesją).

2. **Naprawiony blocker w `createSklepikWorkflow`:** region w Medusie wymaga unikalnego przypisania kraju między regionami — druga (i każda kolejna) próba założenia sklepiku dla Polski wywalała się błędem "Countries with codes: pl are already assigned to a region", bo pierwszy sklepik/instalacja już zajął `pl`. To był realny blocker dla najczęstszego przypadku self-signup (prawie każdy nowy sklepik to Polska). Naprawa: nowy krok `getOrCreateSklepikRegionStep` w `create-sklepik.ts` — jeśli region dla danego kraju już istnieje, reużywa go zamiast tworzyć duplikat (spójne z już udokumentowanym faktem, że region i sales_channel nie mają formalnego powiązania w Medusie — `/store/regions` i tak zwraca wszystkie regiony niezależnie od klucza, więc dzielenie jednego regionu Polski między sklepikami niczego nie psuje). Przetestowane: dwa sklepiki dla `pl` z rzędu, oba działają, drugi reużywa region pierwszego.

3. **🔄 Zaktualizowane (ten sam dzień, druga część sesji): trójwarstwowa struktura domeny `serowymichal.pl`** — pierwotny plan (strona formularza wprost pod `serowymichal.pl`) zmieniony na życzenie właściciela:
   - `serowymichal.pl` / `www.serowymichal.pl` — statyczna "wizytówka" (dziś: sam tytuł + stopka adresowa), katalog `/home/ubuntu/serowymichal-www` na serwerze (świadomie w `~/`, obok innych realnych projektów, nie w `/var/www`). **Uwaga: `/home/ubuntu` wymagał `chmod o+x`, żeby nginx (`www-data`) mógł w ogóle wejść do katalogu** — bez tego 500.
   - `sklepiki.serowymichal.pl` — właściwa strona formularza "Załóż sklepik"/"Zaloguj się" (katalog `/home/ubuntu/sklepiki`), ten sam kod co pierwotnie planowany pod bare domain. Statyczny HTML/CSS/JS, JS woła bezpośrednio `/auth/user/emailpass/register` → `/admin/sklepiki/self-signup` → pokazuje potwierdzenie z linkiem do własnego adresu sklepiku i do panelu admina (`https://141-253-103-172.nip.io/app`). "Zaloguj się" to zwykły link do panelu — landing page **nie** reimplementuje logowania.
   - `nazwa-sklepiku.serowymichal.pl` (dowolna inna subdomena) — proxy do storefrontu (port 8000), middleware (patrz Migration Path krok 6 niżej) rozpoznaje sklepik po handle w subdomenie. Działa dla **każdej** subdomeny dzięki wildcard DNS + wildcard SSL (patrz niżej) — żadnego dodatkowego kroku per sklepik.
   - **DNS w home.pl:** rekordy A dla `@`, `www`, i wildcard `*` → `141.253.103.172`. Jeden wildcard rekord obsługuje zarówno `sklepiki` jak i każdą przyszłą subdomenę sklepiku.
   - **SSL:** ✅ zrobione (nie "do zrobienia" jak mówiła wcześniejsza wersja tej notatki) — certyfikat wildcard `serowymichal-wildcard` (Let's Encrypt, DNS-01 przez `certbot --manual`, ręczny rekord TXT jednorazowo) obejmuje `serowymichal.pl` + `*.serowymichal.pl` (czyli też `sklepiki`). **Odnawianie nie jest automatyczne** (`--manual` nie wspiera auto-renewal bez `--manual-auth-hook`) — do ręcznego powtórzenia przed 2026-10-16.
   - `ADMIN_CORS`/`AUTH_CORS`/`STORE_CORS` w `app/.env` rozszerzone: `ADMIN_CORS`/`AUTH_CORS` o `https://sklepiki.serowymichal.pl` (skąd faktycznie wołane jest API), `STORE_CORS` o wzorzec regex obejmujący dowolną subdomenę (`/^https:\/\/([a-z0-9-]+\.)?serowymichal\.pl$/`).

4. **🔄 Naprawione (druga część sesji): "ładne" adresy sklepików bez losowego sufiksu w większości przypadków.** Do tego momentu `generateSklepikHandle` doklejał losowy sufiks **zawsze** (np. `zajebiscie-db9d05`), nawet gdy nazwa była unikalna w całej instalacji. Naprawa: nowy unikalny indeks w bazie (`IDX_sales_channel_handle_unique`, `Migration20260718170000` w `packages/modules/sales-channel/src/migrations/`) + `getOrCreateSklepikSalesChannelStep` w `create-sklepik.ts` próbuje najpierw czystego sluga, sufiks dokłada tylko gdy baza faktycznie odrzuci wstawienie jako duplikat. Przy okazji naprawiony powiązany bug: nazwa roli RBAC właściciela (`Właściciel: {nazwa}`) kolidowała przy dwóch identycznie nazwanych sklepikach (`rbac_role.name` ma unikalny constraint) — teraz zawiera też id sales_channel. **Uwaga:** istniejące sklepiki sprzed tej naprawy (np. wcześniejszy `zajebiscie-db9d05`) zachowują swój stary adres — nie renumerowane retroaktywnie.

5. **Przetestowane end-to-end na żywym serwerze** (nie tylko czytaniem kodu, kilkukrotnie w trakcie sesji, w tym raz bezpośrednio w przeglądarce przez formularz, nie tylko curlem): rejestracja → self-signup → login → `GET /admin/sklepiki` pokazuje tylko własny sklepik → próba dostępu do cudzego `sales_channel` (istniejący "Sklepik Testowy A") **403** → czysty handle dla unikalnej nazwy, sufiks dla kolizji, oba bez błędu → nowo założony sklepik działa na żywo pod własną subdomeną z HTTPS natychmiast po założeniu. Wszystkie dane testowe za każdym razem posprzątane ręcznie przez SQL (rola `szopifaj` przez `docker exec szopifaj-postgres psql`) — `Sklepik Testowy A` i realne sklepiki właściciela (`zajebiscie`, `Sklepik Pawla testowy`) z poprzednich/równoległych sesji pozostawione nietknięte.

**Do zrobienia:** ewentualny rate-limiting/captcha na publicznym self-signup, jeśli okaże się problemem przy realnym ruchu; ręczne odnowienie certyfikatu wildcard przed 2026-10-16 (patrz punkt 3 wyżej).
...
- ✅ **Rozstrzygnięte 2026-07-18:** rejestracja jest publiczna (Key Decision 7, zmienione). Otwarte pozostaje: czy w przyszłości wprowadzić jakiś rate-limiting/captcha na self-signup, gdy ruch realnie wzrośnie — nie blokuje dzisiejszego MVP.
